...
do\
{\
	assert(this->m_FpVecs.size() == this->pointerVecs.size());\
	assert(this->pointerVecs.size() == this->m_WriteBlockSizeVecs.size());\
}while(0)
...
void CPcmCapDemoCallBack::CloseFile()
{
    ...
    while(this->m_FpVecs.size() > 0)
    {
        ...
        void* pPointer= this->pointerVecs[0];
        int wsize= this->m_WriteBlockSizeVecs[0];

        this->m_FpVecs.erase(this->m_FpVecs.begin());
        this->pointerVecs.erase(this->pointerVecs.begin());
...
int CPcmCapDemoCallBack::__WriteFile(pcmcap_buffer_t * pPcmItem,LPVOID lpParam)
{
    unsigned int i;
    int findidx=-1;
    int removeidx=-1;
    ...
    pSearchPointer = (void*)pPcmItem->data.pointer;
    for(i=0; i<this->pointerVecs.size(); i++)
    {
        if(pSearchPointer == this->pointerVecs[i])
        {
            findidx = i;
            break;
        }
    ...
    /*now to write*/
    ret = fwrite(pPcmItem->data.data,pPcmItem->data.datalen,1,pWriteFp);
    if(ret != 1)
    {
        ret = LAST_ERROR_CODE();
        _snprintf_s((char*)fnametmp,sizeof(fnametmp),_TRUNCATE,"%s.0x%p",this->m_FileNameBase,pSearchPointer);
    ...
    writesize += pPcmItem->data.datalen;
    ...
    else
    {
        ...
        this->pointerVecs.push_back(pSearchPointer);
    }
...
fail:
    if(findidx >= 0)
    {
        ...
        this->pointerVecs.erase(this->pointerVecs.begin() + findidx);
...
int CPcmCapDemoCallBack::__StartPcmPlay(pcmcap_format_t * pFormat)
{
	return 1;
}

int CPcmCapDemoCallBack::__PcmPlay(pcmcap_format_t * pFormat,unsigned char * pBuffer,int bytes)
{
	int ret;
	...
	pFormatEx = (WAVEFORMATEX*) pFormat->format;
	size = sizeof(*pFormatEx);
	size += pFormatEx->cbSize;
	if (size > sizeof(pFormat->format))
	{
		ret = ERROR_INSUFFICIENT_BUFFER;
		ERROR_INFO("the format size %d > sizeof(%d)\n",size,sizeof(pFormat->format));
		goto fail;
	}
...
void CPcmCapDemoCallBack::__InnerPcmPlay(pcmcap_buffer_t * pPcmItem,LPVOID lpParam)
{
	int ret ;

	ret = this->__StartPcmPlay(&(pPcmItem->format));
	if (ret < 0)
	{
		this->__StopPcmPlay();
		return ;
	...
	ret = this->__PcmPlay(&(pPcmItem->format),pPcmItem->data.data,pPcmItem->data.datalen);
	if (ret < 0)
	{
		this->__StopPcmPlay();
		return ;
...
VOID CPcmCapDemoCallBack::WaveInCb(pcmcap_buffer_t * pPcmItem,LPVOID lpParam)
{
#if 1
