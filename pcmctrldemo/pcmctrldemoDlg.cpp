...
CpcmctrldemoDlg::CpcmctrldemoDlg(CWnd* pParent /*=NULL*/)
...
BEGIN_MESSAGE_MAP(CpcmctrldemoDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
...
void CpcmctrldemoDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//显示。
HCURSOR CpcmctrldemoDlg::OnQueryDragIcon()
{
...
int CpcmctrldemoDlg::StartProcessCapper(unsigned int procid,int rendercheck,int capturecheck,int bufnum,int blocksize,const char * pDumpAnsi)
{
    ...
    /*now to get the text  */
    this->m_pCapper = new CPcmCapController();
