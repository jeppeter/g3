...
static int HoldDevice(const char* file,const char*func,int lineno,ID3D11Device *pDevice)
{
    ...
    do
    {
        ...
        if(holdidx.size() > 0)
        {
            ...
            {
                idx = holdidx[i];
                assert(idx < (int)st_D11PointersVec.size());
                pCurPointer = st_D11PointersVec[idx];
...
int __CaptureBufferDX11(ID3D11Device *pDevice,ID3D11DeviceContext* pContext,IDXGISwapChain* pSwapChain,HANDLE hRemoteProc,void* pRemoteAddr,int remotelen,unsigned int *pFormat,unsigned int* pWidth,unsigned int* pHeight)
{
    ...
    *pFormat = (unsigned int)__TransDXGI_FORMAT(StagingDesc.Format);
    *pWidth = StagingDesc.Width;
...
int CaptureBufferDX11(imgcap_buffer_t* pCapture)
{
    int idx=0,res;
    ID3D11Device *pDevice=NULL;
    ...
    while(1)
    {
		...
		DEBUG_INFO("get[%d]\n",idx);
        ret = __CaptureBufferDX11(pDevice,pContext,pSwapChain,hRemoteProc,pCapture->data,pCapture->datalen,&(pCapture->format),&(pCapture->width),&(pCapture->height));
        if(ret >= 0)
        {
            getlen = ret;
            DEBUG_INFO("getlen %d\n",getlen);
