...
do\
{\
	assert(st_hWndBaseRectVecs.size() == st_hWndBaseVecs.size());\
	assert(st_hWndClassCursorVecs.size() == st_hWndBaseVecs.size());\
}while(0)
...
ULONG_PTR WINAPI GetClassLongPtrACallBack(HWND hwnd,int nIndex)
{
    ...
    {
        pRet = GetClassLongPtrANext(hwnd,nIndex);
    }
    return pRet;
}
    ...
    if(hwnd && nIndex == GCLP_HCURSOR && st_ShowCursorInit)
    {
        ...
        if(findidx >= 0)
        {
            if(st_ShowCursorHideMode == 0)
            {
                ...
                pRet =  SetClassLongPtrANext(hwnd,GCLP_HCURSOR,dwNewLong);
                if(pOldRet != pRet)
                {
...
BOOL RemoveHwnd(HWND hwnd)
{
    ...
    if(st_ShowCursorInit)
    {
        ...
        if(findidx >= 0)
        {
            ...
            st_hWndBaseRectVecs.erase(st_hWndBaseRectVecs.begin() + findidx);
            bret = TRUE;
            ret = 0;
        }
        else
    ...
    else
    {
        for(i=0; i<st_hWndBaseRectVecs.size(); i++)
        {
            ...
            if(bret)
            {
                pt.x = rRect.left;
                pt.y = rRect.top;
                bret = ::ClientToScreen(st_hWndBaseVecs[i],&pt);
                if(!bret)
                {
                    ret = LAST_ERROR_CODE();
                    ERROR_INFO("ClientToScreen(%d-%d) Error(%d)\n",pt.x,pt.y,ret);
