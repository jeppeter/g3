...
static int SetFormat(WAVEFORMATEX* pFormatEx)
{
    ...
    EnterCriticalSection(&st_StateCS);
    if(size <= sizeof(st_AudioFormat.format))
    {
        memcpy(st_AudioFormat.format,pFormatEx,size);
    }
    else
    {
        error =1;
        memcpy(st_AudioFormat.format,pFormatEx,sizeof(st_AudioFormat.format));
    }
    LeaveCriticalSection(&st_StateCS);

    if(error)
    {
        ERROR_INFO("need size (%d) > sizeof(%d)\n",size,sizeof(st_AudioFormat.format));
...
void __InitThreadControl(pcmcap_thread_control_t* pThreadControl)
{
    ...
    pThreadControl->exitevt = NULL;
    pThreadControl->running = 0;
...
void __StopThread(pcmcap_thread_control_t* pThreadControl,int force)
{
    ...
    if(pThreadControl)
    {
        if(pThreadControl->thread)
        {
            pThreadControl->running = 0;
            assert(pThreadControl->exitevt);
            while(pThreadControl->exited == 0)
            {
                bret = SetEvent(pThreadControl->exitevt);
                if(!bret)
                {
                    ret = LAST_ERROR_CODE();
                    ERROR_INFO("could not setevent 0x%x error(%d)\n",pThreadControl->exitevt,
                               ret);
                }
                if((printout % 100)==0)
                {
        ...
        if(pThreadControl->thread)
        {
            DEBUG_INFO("thread 0x%x\n",pThreadControl->thread);
            CloseHandle(pThreadControl->thread);
        }
        pThreadControl->thread = NULL;
        if(pThreadControl->exitevt)
        {
            DEBUG_INFO("hExitEvt 0x%x\n",pThreadControl->exitevt);
            CloseHandle(pThreadControl->exitevt);
        }
        pThreadControl->exitevt = NULL;
...
int __StartThread(pcmcap_thread_control_t* pThreadControl,ThreadFunc_t pStartFunc,void* pParam)
{
    ...
    if(pThreadControl == NULL || pStartFunc == NULL || pThreadControl->running != 0 ||
            pThreadControl->thread != NULL || pThreadControl->threadid != 0
            || pThreadControl->exitevt != NULL || pThreadControl->exited != 1)
    {
    ...
    pThreadControl->exitevt = GetEvent(NULL,1);
    if(pThreadControl->exitevt == NULL)
    {
        ret = LAST_ERROR_CODE();
    ...
    pThreadControl->running = 1;
    pThreadControl->exited = 0;
    pThreadControl->thread = CreateThread(NULL,0,pStartFunc,pParam,0,&(pThreadControl->threadid));
    if(pThreadControl->thread == NULL)
    {
        pThreadControl->exited = 1;
        pThreadControl->running = 0;
        ret = LAST_ERROR_CODE();
...
PCM_EVTS_t* __AllocatePCMEvts(unsigned int num,int packsize,char* pMapFileName,char* pFreeEvtNameBase,char* pFillEvtNameBase,char* pStartEvtName,char* pStopEvtName)
{
    ...
    for(i=0; i<num; i++)
    {
        ...
        pPCMEvt->m_pWholeList[i].size = packsize;
...
static int GetFormat(IAudioRenderClient* pRender,pcmcap_format_t *pAudioFormat);
...
int WriteSendBuffer(IAudioRenderClient *pClient,unsigned char* pBuffer,int numpacks)
{
    ...
    pcmcap_buffer_t* pAudioBuffer=NULL;
    pcmcap_format_t  audioformat;
    ...
    pAudioFormat = (WAVEFORMATEX*)audioformat.format;
    ...
    if(numbytes > (pEventList->size - sizeof(audioformat)-sizeof(pAudioBuffer->data) + sizeof(pAudioBuffer->data.data)))
    {
        ERROR_INFO("Exceeded size (%d) > (%d - %d - %d + %d)\n",numbytes,pEventList->size ,sizeof(audioformat),
                   sizeof(pAudioBuffer->data),sizeof(pAudioBuffer->data.data));
        PutFreeList(pEventList);
        return 0;
    }

    ret = WriteShareMem((unsigned char*)pAudioBuffer->data.data,0,pBuffer,numbytes);
    if(ret < 0)
    {
    ...
    ret = WriteShareMem((unsigned char*)&(pAudioBuffer->data.datalen),0,(unsigned char*)&(numbytes),sizeof(numbytes));
    if(ret < 0)
    {
    ...
    ret = WriteShareMem((unsigned char*)&(pAudioBuffer->data.pointer),0,(unsigned char*)&(pClient),sizeof(pAudioBuffer->data.pointer));
    if(ret < 0)
    {
    ...
    //DEBUG_BUFFER(pAudioBuffer->data.data,numbytes > 16 ? 16 : numbytes);
...
DWORD WINAPI __WaitThreadImpl(LPVOID arg)
{
    ...
    pWaitHandles[num] = pPCMEvts->m_ThreadControl.exitevt;

    while(pPCMEvts->m_ThreadControl.running)
    {
...
out:
    ...
    pWaitHandles = NULL;
    pPCMEvts->m_ThreadControl.exited = 1;
    return (DWORD)ret;
}
...
int __HandleAudioRecordStart(pcmcap_control_t *pControl)
{
    ...
    ret = InitializeWholeList(pControl->packnum,pControl->packsize ,
                              (char*)pControl->mem_sharename,(char*)pControl->freelist_semnamebase,(char*)pControl->filllist_semnamebase,
                              (char*)pControl->startevt_name,(char*)pControl->stopevt_name);
    if(ret < 0)
    {
        ret = -ret;
        goto fail;
...
{
    int ret = 0;
    unsigned int tmms = pControl->timeout * 1000;
    DWORD dret;
...
static int GetFormat(IAudioRenderClient* pRender,pcmcap_format_t *pAudioFormat)
{
    ...
    if(findidx >= 0)
    {
        ...
        size += pFormat->cbSize;
        if(size >= sizeof(pAudioFormat->format))
        {
            memcpy(pAudioFormat->format,pFormat,sizeof(pAudioFormat->format));
        }
        else
        {
            memcpy(pAudioFormat->format,pFormat,size);
        }
    ...
    {
        EnterCriticalSection(&st_StateCS);
        pAudioFormat->volume = st_AudioFormat.volume;
        LeaveCriticalSection(&st_StateCS);
    }
    return ret;
}
...
void PcmCapInjectFini(void)
{
    if(st_PcmCapInited)
    {
        ...
        do
        {
            ...
            st_DummyControl.operation = PCMCAP_AUDIO_NONE;
