...
public:
    ...
    COM_METHOD(HRESULT,EnumTypes)(THIS_ LPDIJOYTYPECALLBACK lpCallback,LPVOID pvRef)
    {
        ...
        hr = this->m_ptr->EnumTypes(CDirectInputJoyConfigHook::DIEnumJoyTypeCallback,this);
        DINPUT_JOYCONFIG_OUT();
        return hr;
    }
	...
	COM_METHOD(HRESULT,SetTypeInfo)(THIS_ LPCWSTR pwszTypeName,LPCDIJOYTYPEINFO pjti,DWORD dwFlags)
    ...
    COM_METHOD(HRESULT,OpenConfigKey)(THIS_  UINT idJoy,REGSAM regsam,PHKEY phk)
    {
        HRESULT hr;
        DINPUT_JOYCONFIG_IN();
        hr = this->m_ptr->OpenConfigKey(idJoy,regsam,phk);
        DINPUT_JOYCONFIG_OUT();
        return hr;
    }
};
...
CDirectInputJoyConfigHook* RegisterJoyConfigHook(IDirectInputJoyConfig* pJoyConfig)
{
    CDirectInputJoyConfigHook* pHook=NULL;
    UINT i;
    int findidx=-1;
    EnterCriticalSection(&st_DIDevice8ACS);
    JOY_CONFIG_ASSERT();
    for(i=0; i<st_DIJoyConfigVecs.size(); i++)
    {
        if(st_DIJoyConfigVecs[i] == pJoyConfig)
        {
            findidx = i;
            break;
        }
    ...
    if(findidx >= 0)
    {
        pHook = st_CDIJoyConfigHookVecs[findidx];
    }
    else
    {
        pHook = new CDirectInputJoyConfigHook(pJoyConfig);
        pJoyConfig->AddRef();
        st_CDIJoyConfigHookVecs.push_back(pHook);
        st_DIJoyConfigVecs.push_back(pJoyConfig);
    }
    LeaveCriticalSection(&st_DIDevice8ACS);
...
public:
    ...
    COM_METHOD(HRESULT,EnumCreatedEffectObjects)(THIS_  LPDIENUMCREATEDEFFECTOBJECTSCALLBACK lpCallback,LPVOID pvRef,DWORD fl)
    {
        ...
        hr = m_ptr->EnumCreatedEffectObjects(CDirectInputDevice8AHook::DIEnumCreatedEffectObjectsCallback,this,fl);
        DIRECT_INPUT_DEVICE_8A_OUT();
        return hr;
    }
    ...
    COM_METHOD(HRESULT,EnumEffectsInFile)(THIS_ LPCSTR lpszFileName,LPDIENUMEFFECTSINFILECALLBACK pec,LPVOID pvRef,DWORD dwFlags)
    {
        ...
        hr = m_ptr->EnumEffectsInFile(lpszFileName,CDirectInputDevice8AHook::DIEnumEffectsInFileCallback,this,dwFlags);
        DIRECT_INPUT_DEVICE_8A_OUT();
        return hr;
    }
...
public:
    ...
    COM_METHOD(HRESULT,EnumObjects)(THIS_ LPDIENUMDEVICEOBJECTSCALLBACKW lpCallback,LPVOID pvRef,DWORD dwFlags)
    {
        ...
        this->m_pEnumDeviceCallback = lpCallback;
        this->m_pEnumDeviceVoid = pvRef;
        hr = m_ptr->EnumObjects(CDirectInputDevice8WHook::DIEnumDeviceObjectsCallback,this,dwFlags);
        DIRECT_INPUT_DEVICE_8W_OUT();
        return hr;
    }
